 ...
 *
 * Mobile build: dpr=1 and no inset drawImage/clip for reliable rendering on mobile.
 */
(function () {
  'use strict';
  ...
  var canvasHeight = 0;
  var rafId = null;
  var dpr = 1; /* mobile: avoid HiDPI canvas issues */
  ...
  function drawSensor(ctx, xSensor, labelY, mainCy, mainHeight, labelX) {
    var cy = mainCy != null ? mainCy : canvasHeight / 2;
    ...
    ctx.stroke();
    var textX = (labelX != null ? labelX : xSensor);
    ctx.font = 'bold 14px sans-serif';
    ctx.fillStyle = '#363636';
    ctx.textAlign = 'center';
    ctx.fillText('Grayscale Sensor', textX, labelY != null ? labelY : cy + h + 18);
  }

  function drawWavelengthLabel(ctx, xSensor, labelY, labelX) {
    var y = (labelY != null ? labelY + 22 : canvasHeight / 2 + canvasHeight * UI.sensor_half_height_ratio + 38);
    var textX = (labelX != null ? labelX : xSensor);
    ctx.font = 'bold 12px sans-serif';
    ctx.fillStyle = '#363636';
    ctx.textAlign = 'center';
    ctx.fillText('λ in focus: ' + Math.round(getFocusWavelength()) + ' nm', textX, y);
  }
  ...
  function drawInsetConnectors(ctx, xSensor, cy, radius, insetX, insetY, insetW, insetH) {
    /* mobile: skip tangent connectors to avoid rendering glitches */
  }

  /**
   * Mobile: draw only the inset border and label (no drawImage/clip for reliability).
   */
  ...
  function draw() {
    ...
    var gap = 8;
    /* Mobile: reserve bottom for PSFs; center optics in the top region only.
     * Reserve enough so "Simulated Sample PSF's" title (extends ~18px above baseline) clears optics. */
    var blockH = UI.psf_size_px + 36;
    var reserveBottom = blockH + 62;
    var opticsRegionHeight = canvasHeight - reserveBottom;
    var cy = opticsRegionHeight / 2;
    /* Mobile: leave right margin for labels; keep L1, L2, sensor in same relative positions */
    var rightLabelMargin = 56;
    var opticsEnd = canvasWidth - margin - rightLabelMargin;
    ...
    /* Use optics region height for vertical scale so optics stay above PSF block */
    var arrowHalfH = opticsRegionHeight * UI.lens_half_height_ratio * 0.9;
    var sensorH = opticsRegionHeight * UI.sensor_half_height_ratio * 0.9;
    var labelY = cy + Math.max(arrowHalfH, sensorH) + 18;
    var distanceY = cy - arrowHalfH + 4;

    var psfLayoutRight = false; /* mobile: always draw PSFs below */

    ctx.clearRect(0, 0, canvasWidth, canvasHeight);

    drawOpticalAxis(ctx, margin, xSensor, cy);
    ...
    var rayResult = drawRays(ctx, xL1, xL2, xSensor, scale, L2_mm, cy, opticsRegionHeight);
    ...
    drawThinLensArrow(ctx, xL1, 'Objective Lens', labelY, cy, opticsRegionHeight, OPTICS.focal_length_550_nm_mm);
    drawThinLensArrow(ctx, xL2, 'Moving Lens', labelY, cy, opticsRegionHeight, OPTICS.focal_length_550_nm_mm);
    var sensorLabelX = xSensor + 32;
    drawSensor(ctx, xSensor, labelY, cy, opticsRegionHeight, sensorLabelX);
    drawWavelengthLabel(ctx, xSensor, labelY, sensorLabelX);

    /* Mobile: skip inset (circle, connectors, zoom); PSFs below */
    drawPSFPlaceholders(ctx, false, 0);
  }
  ...
  function resize() {
    ...
    if (w <= UI.mobile_breakpoint_px) h = Math.max(420, Math.floor(w * 1.05));
